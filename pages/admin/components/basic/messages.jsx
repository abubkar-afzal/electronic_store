...
const Messages = () => {
  ...
  const Loader = () => (
    <div className="text-center text-gray-500 italic py-4 animate-pulse">
      Loading...
    </div>
  );
  ...
  const Section = ({ title, data, loading, fields }) => (
    <div className="bg-white p-6 rounded-lg shadow mb-10 w-full">
      <h2 className="text-2xl font-bold mb-4 border-b pb-2 text-[var(---btncolor)]">
        {title}
      </h2>
      {loading ? (
        <Loader />
            ...
            <thead>
              <tr>
                {fields.map((field) => (
                  <th
                    key={field}
                    className="border px-4 py-2 bg-gray-100 text-left capitalize"
                  >
                    {field.replace("_", " ")}
                  </th>
                ))}
